...
fn load_and_preprocess_dataset(file_path: &str) -> Result<(Array2<f64>, Vec<i64>), Box<dyn Error>> {
    ...
    let scaled_features = basic_scale_features(features_array);
...
// Function for basic scaling
fn basic_scale_features(mut features: Array2<f64>) -> Array2<f64> {
    for mut col in features.axis_iter_mut(Axis(1)) {
        let max = col.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        col.map_inplace(|x| *x = *x / max); // Scale by dividing by max
    }
    features
}
